//g++ SSBCalibration.c -Wl,--no-as-needed `root-config --cflags --libs --glibs` -lSpectrum -lMinuit -lGuiHtml -lTreePlayer -lTMVA -L/opt/local/lib -lX11 -lXpm -O2 -Wl,--copy-dt-needed-entries -L/opt/local/lib -lX11 -lXpm `grsi-config --cflags --all-libs --GRSIData-libs` -I$GRSISYS/GRSIData/include -o Alphacal
...
std::vector<Double_t> PeakHunt(TH1 *hist){
  ...
  hist->GetXaxis()->SetRangeUser(60,1e3); // skip the noisy peak
  ...
  const double min_spacing = 1.0; // Peaks must be separated by at least 5 ADC channels
...
// Fill hists and TF1*fit to SSBCalibrationOutputs.root
// Save gain[] and offsets[] array to SSBCalibration.txt
void CalHist(int minCH, int maxCH, Option_t *opt=""){
  ...
  outfile.open("SSBCalibration.txt");
  ...
  outfile << "};";
  outfile.close();
  std::cout << "Writing gains and offsets to " << "SSBCalibration.txt" << std::endl;
...
int main(int agrc, char **agrv){
  ...
  outfile = "SSBCalibrationOutputs.root";
