// g++ S3DeadLayer.cxx -std=c++0x -I$GRSISYS/include -L$GRSISYS/lib `grsi-config --cflags --all-libs --GRSIData-libs` -I$GRSISYS/GRSIData/include -L$GRSISYS/GRSIData/lib `root-config --cflags --libs` -o S3DeadLayer
...
void S3DeadLayerCalc(double DLSlopeCoeff) {
    ...
    std::cout << "Energy loss of " << alphaEnergy << " keV alpha in Si " << dEdx << " keV/um" << std::endl;

    double DLThickness = - DLSlopeCoeff / dEdx; // Dead layer thickness in um
    std::cout << "Dead layer thickness: " << DLThickness << " um" << std::endl;
}
...
void S3DeadLayer() {
    ...
    TDirectory *dir = inputFile->GetDirectory("S3EnergyPlots");
    ...
    TH2F *h_sectorE_v_ring_num = (TH2F*)dir->Get("h_sectorE_v_ring_num_after_cal");
    h_sectorE_v_ring_num->Draw("COLZ");
    h_sectorE_v_ring_num->GetYaxis()->SetRangeUser(4500,6500);
    c1->SetLogz();
    c1->SaveAs("SectorEVRingNum.pdf");
    ...
    TH2F *h_sectorE_v_inv_cos = new TH2F("h_sectorE_v_inv_cos", 
                                        "Sector Energy vs 1/cos(#theta); 1/cos(#theta); Sector Energy (1 keV bins)", 
                                        24, // 1/cos(theta) bins
                                        xbins, // 1/cos(theta) bin edges
                                        10000, // Sector energy bins
    ...
    TCanvas *c2 = new TCanvas("c2", "Sector Energy vs 1/cos(theta)", 1400, 600);
    h_sectorE_v_inv_cos->Draw("COLZ");
    h_sectorE_v_inv_cos->GetYaxis()->SetRangeUser(4500,6500);
    c2->SetLogz();
    c2->SaveAs("SectorEVInvCos.pdf");
    ...
    h_sectorE_v_inv_cos->GetYaxis()->SetRangeUser(4950,5250);
    ...
    h_sectorE_v_inv_cos->GetYaxis()->SetRangeUser(5400,5600);
    ...
    h_sectorE_v_inv_cos->GetYaxis()->SetRangeUser(5700,5900);
    ...
    c2->Update();
        ...
        h_sectorE_v_inv_cos->GetZaxis()->SetTitle("Counts");
    h_sectorE_v_inv_cos->GetZaxis()->SetTitleOffset(1.0);

    c2->SaveAs("FittedAlphaCurves.root");

 
    // ------------------------------------------------------------
    // Remove the dead-layer slope using the fitted 244Cm line.
    //
    // Fit has the form:
    ...
    double Pu_slope     = PuFit->GetParameter(1);
    double Am_intercept = AmFit->GetParameter(0);
    double Am_slope     = AmFit->GetParameter(1);
    double Cm_intercept = CmFit->GetParameter(0);
    double Cm_slope     = CmFit->GetParameter(1);
    ...
    TH2F *h_sectorE_v_inv_cos_deadlayer_removed = new TH2F("h_sectorE_v_inv_cos_deadlayer_removed",
                "Sector Energy vs 1/cos(#theta), dead layer corrected;"
                "1/cos(#theta); Sector Energy (1 keV bins)",
                24,
                xbins,
                10000,
                0,
    ...
    for (int i = 1; i <= h_sectorE_v_inv_cos->GetNbinsX(); i++) {
        ...
        //
        // For the fitted alpha source line:
        //     E = E0 + m*x
        //
    ...
    c3->SetLogz();
    c3->Update();
    c3->SaveAs("SectorEVInvCosDeadLayerRemoved.pdf");

    S3DeadLayerCalc(Pu_slope);
    //S3DeadLayerCalc(Am_slope);
    //S3DeadLayerCalc(Cm_slope);
}
